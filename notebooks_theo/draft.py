...
rng = jax.random.PRNGKey(seed)
latent_batch = jax.random.normal(
    rng,
    shape=(len(batch['source']), output_dim)
)
mu_0, std_0 = jnp.zeros_like(latent_batch), jnp.ones_like(latent_batch)
print(
    mu_0.shape, 
    std_0.shape,
    latent_batch.shape
...
apply_fn_partial = jax.tree_util.Partial(
    vector_field_net.apply, condition=x
)
