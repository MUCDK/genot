...
class NoiseOutsourcingModel:
    def __init__(
        ...
        **kwargs,
        ...
        t = MLP(dim_hidden=[128, 128, 128, 128], is_potential=False, noise_dim=noise_dim, act_fn=nn.relu)

        phi = MLP(dim_hidden=[128, 128, 128, 128], is_potential=True, act_fn=nn.relu)
    ...
    def train(self, batch: Dict[str, jnp.ndarray], epsilon: float, key: jax.random.PRNGKeyArray) -> Dict[str, Any]:
        ...
        pi_star_inds = jax.random.categorical(
            key, logits=jnp.log(out.matrix.flatten()), shape=(self.batch_size_source,)
        )
...
class GromovNOM(NoiseOutsourcingModel):
    ...
    def train(self, batch: Dict[str, jnp.ndarray], epsilon: float, key: jax.random.PRNGKeyArray) -> Dict[str, Any]:
        ...
        out = gromov_wasserstein.GromovWasserstein(epsilon=epsilon)(
            quadratic_problem.QuadraticProblem(geom_xx, geom_yy)
        )
        pi_star_inds = jax.random.categorical(
            key, logits=jnp.log(out.matrix.flatten()), shape=(self.batch_size_source,)
        )
...
class KantorovichGapModel:
    def __init__(
        ...
        unbalanced_kwargs: Optional[Dict[str, Any]] = None,
        rng: Optional[jax.random.PRNGKeyArray] = None,
    ) -> None:
        self.input_dim = input_dim
        ...
        self.kant_gap = (
            KantorovichGap(
                noise_dim=noise_dim, geometry_kwargs={"epsilon": epsilon, **{"cost_fn": costs.SqEuclidean()}}
            )
            if kantorovich_gap is None
            else kantorovich_gap
        )
    ...
    def __call__(self, x: Union[jnp.ndarray, MixtureNormalSampler], y: Union[jnp.ndarray, MixtureNormalSampler]) -> Any:
        batch: Dict[str, jnp.ndarray] = {}
        self.x_loader = (
            DataLoader(x, batch_size=self.batch_size_source) if not isinstance(x, MixtureNormalSampler) else x
        )
        self.y_loader = (
            DataLoader(y, batch_size=self.batch_size_target) if not isinstance(y, MixtureNormalSampler) else y
        )
        for step in tqdm(range(self.iterations)):
            # for step in range(self.iterations):
            self.rng, rng_x, rng_y, rng_noise, rng_sample = jax.random.split(self.rng, 5)
    ...
    def _get_step_fn(self) -> Callable:
        def loss_fn(
            ...
            batch: Dict[str, jnp.ndarray],
            unbalanced_kwargs: Optional[Dict[str, Any]],
        ) -> Tuple[float, Dict[str, float]]:
            t_xz = apply_fn_mlp({"params": params_mlp}, batch["source"])

            if unbalanced_kwargs is not None:
                ...
                sinkhorn_output_unbalanced = (None, None)

            batch["a"] = sinkhorn_output_unbalanced.matrix.sum(axis=1) * len(batch["source"])
            batch["b"] = sinkhorn_output_unbalanced.matrix.sum(axis=0) * len(batch["target"])
        ...
        def step_fn(
            ...
            argnums = (0, _ if state_eta is None else 1, _ if state_xi is None else 2)
            argnums = argnums + (1,) if state_eta is not None else argnums
            argnums = argnums + (2,) if state_xi is not None else argnums
