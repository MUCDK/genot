...
def sample_indices_from_tmap(
    ...
    n_samples = n_samples or tmat.shape[1]
...
class OTFlowMatching:
    def __init__(
        ...
        callback_fn: Optional[
            Callable[
                [Solver, Dict[str, jnp.ndarray], Dict[str, jnp.ndarray]], Any
            ]
        ] = None,
        callback_kwargs: Dict[str, Any] = {},
        callback_freq: int = 1_000,
        ...
        **kwargs: Any,
        ...
        if (
            ...
            raise ValueError(
                "If `ot_solver` is `GromovWasserstein`, `epsilon` "
                "must be `None`. This check is performed to ensure that in "
                "the (fused) Gromov case the `epsilon` parameter is passed "
                "via the `ot_solver`."
            )
        ...
        self.optimizer = optimizer or optax.adamw(
            learning_rate=1e-4, weight_decay=1e-10
        ) 
    ...
    def setup(self, **kwargs: Any) -> None:
        ...
        # setup wandb and callback function
        if self.callback_fn:
            assert self.test_batch is not None, (
                "A test batch must be passed when using a callback function."
            )
        if self.log_wandb:
            assert self.callback_fn, (
                "a callback function must be passed when log_wandb is True."
            )

    def setup_data(
        self, 
        z: Union[jnp.array, Iterator],
        batch_size: Optional[int] = None,
    ) -> Tuple[Iterator, Callable[[jnp.array], jnp.array]]:
        if not hasattr(z, "shape"):
            z_loader, z_load_fn = z, lambda z: z
        else:
            assert batch_size is not None, (
                "'batch_size_source' must be specified when 'x' is an array."
            )
            z_loader = iter(
                tf.data.Dataset.from_tensor_slices(z).repeat().shuffle(
                    buffer_size=10_000, 
                    seed=0
                ).batch(batch_size)
            )
            z_load_fn = lambda z: jnp.asarray(tfds.as_numpy(z))
        return z_loader, z_load_fn
        
    def __call__(
        self,
        x: Union[jnp.array, Iterator],
        y: Union[jnp.array, Iterator],
        batch_size_source: Optional[int] = None,
        batch_size_target: Optional[int] = None,
    ) -> Any:
        
        # setup data
        x_loader, x_load_fn = self.setup_data(x, batch_size_source)
        y_loader, y_load_fn = self.setup_data(y, batch_size_target)
        ...
        for step in tqdm(range(self.iterations)):
            ...
            is_callback_step = (
                self.callback_fn 
                and 
                (step % self.callback_freq == 0)
            )
            for key, value in metrics.items():
                self.logs[key].append(value)
                if is_callback_step and self.log_wandb:
                    wandb.log({"train_"+key: value})    
                
            if is_callback_step:
                out = self.callback_fn(
                    solver=self, # to use self.transport inside callback_fn
                    train_batch=batch, # take last training batch
    ...
    def _get_match_fn_graph(
        ...
        **kwargs,
        ...
        def get_nearest_neighbors(
            ...
            pairwise_sq_euclidean_distances = pointcloud.PointCloud(concat, concat).cost_matrix
            distances, indices = jax.lax.approx_min_k(
                pairwise_sq_euclidean_distances, k=k, recall_target=0.95, aggregate_to_topk=True
            )
...
class simpler_OTFlowMatching:
    ...
    def __call__(
        ...
        for step in tqdm(range(self.iterations)):
            ...
            batch["time"] = t[:, None]
            batch["noise"] = self.noise_fn(
                rng_noise, shape=(len(source_batch),)
            )
            (
                metrics,
                self.state_neural_net,
    ...
    def _get_sinkhorn_match_fn(
        ...
        @partial(
            jax.jit,
            static_argnames=[
                "ot_solver", "epsilon", "cost_fn", 
                "scale_cost", "tau_a", "tau_b", 
            ],
        )
        def match_pairs(
            key: jax.random.PRNGKeyArray,
            ...
            geom = geometry.Geometry(
                cost_matrix=cost_matrix, epsilon=epsilon, 
                scale_cost=scale_cost, 
            )
            out = ot_solver(
            ...
            inds_source, inds_target = sample_indices_from_tmap(
                key=key, tmat=out.matrix, n_samples=len(x)
            )
            return x[inds_source], y[inds_target], a, b
    ...
    def _get_step_fn(self) -> Callable:
        ...
        def step_fn(
            ...
            batch["noise"] = jnp.reshape(batch["noise"], (len(source_batch), -1))
