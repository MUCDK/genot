...
@hydra.main(config_path=config_dir, config_name="scRNAseq.yaml")
def evaluate(cfg: DictConfig) -> None:
    ...
    x_train = full_ds.train.source.adata.to_df().values
    y_train = full_ds.train.target.adata.to_df().values 
    if cfg.data.where == "latent_space":
        if cfg.callback.evaluation_in_data_space:
            full_dataspace_ds = utils_cellot.load_dataset(
                drug_name=cfg.data.drug_name,
                drug_setting=cfg.data.drug_setting,
                where="data_space",
                cellot_path=cellot_path,
            )
            x_test = full_dataspace_ds.test.source.adata.to_df().values
            y_test = full_dataspace_ds.test.target.adata.to_df().values
        else:
            x_test = full_ds.test.source.adata.to_df().values
            y_test = full_ds.test.target.adata.to_df().values
